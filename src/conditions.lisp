...
(define-condition geonames-request-error (geonames-error)
  ((code :reader code
   ...
   (message :reader message
            :initarg :message
            :documentation "Explanation message."))
  (:documentation "Exception raised when HTTP return code after performed 
the request isn't 200.")
  (:report (lambda (condition stream)
...
(define-condition geonames-output-error (geonames-error)
  ...
  (:documentation "Exception raised when the client requests data is in 
an unknown format.")
  (:report (lambda (condition stream)
